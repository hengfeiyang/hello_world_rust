...
impl ObjectStoreProvider for DatafusionCliObjectStoreProvider {
    fn get_by_url(&self, url: &Url) -> Result<Arc<dyn object_store::ObjectStore>> {
        ObjectStoreScheme::from_str(url.scheme()).map(|scheme| match scheme {
            ...
            ObjectStoreScheme::MEM => build_mem_object_store(url),
        })?
    }
}
...
fn build_mem_object_store(url: &Url) -> Result<Arc<dyn object_store::ObjectStore>> {
    println!(
        "Using in-memory object store (not persistent): url: {}",
        url
    );
    let mut builder = mystore::InMemory::new();
    builder.with_path(url.to_string());
    Ok(Arc::new(builder))
}
