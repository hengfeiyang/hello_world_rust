...
pub fn ben_benchmark(c: &mut Criterion) {
    ...
    let keyword = KEYWORD;
    let finder = memchr::memmem::Finder::new(keyword);
    for alias in ["std", "simd", "memchr", "simd-cached"] {
        let h = match alias {
            "std" => std::find,
            "simd" => simd::find,
            "memchr" => simd::find,
            "simd-cached" => simd::find,
            _ => panic!("not support version"),
        };
        group.bench_function(BenchmarkId::from_parameter(format!("{alias}-find")), |b| {
            b.iter(|| {
                if alias == "memchr" {
                    let _ = finder.find(black_box(&body.as_bytes()));
                } else if alias == "simd-cached" {
                    let _ = simd::find_cached(black_box(&body), black_box(&keyword));
                } else {
